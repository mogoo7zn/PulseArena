...
def update_catalog(manifest_path: Path, model_id: str, label: str, promote_default: bool) -> None:
...
def main() -> int:
    ...
    if args.update_catalog or args.promote_default:
        update_catalog(manifest_path, args.model_id, str(manifest["label"]), args.promote_default)
    print(json.dumps({
        "imported": True,
        "model_id": args.model_id,
        "checkpoint": str(checkpoint_path.relative_to(ROOT)).replace("\\", "/"),
