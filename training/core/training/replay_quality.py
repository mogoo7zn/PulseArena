"""Hybrid Tactical v2 replay quality audit + episode-based dataset splitter.

Used by the BC trainer to validate incoming data and build non-leaking
train/val splits that respect episode boundaries.
"""
...
def build_episode_split(
    rows: Iterable[Mapping[str, Any]], holdout_ratio: float, seed: int
) -> dict[str, set[str]]:
...
def audit_tactical_data(replay_dir: Path, output: Path) -> dict[str, Any]:
        ...
        with path.open("r", encoding="utf-8", errors="replace") as handle:
            for line_number, line in enumerate(handle, start=1):
                ...
                if (
                    ...
                    or row.get("replay_schema") != "hybrid_replay_v2"
                    or not _has_required_fields(row)
                ):
                    malformed_rows += 1
                    file_malformed += 1
                    continue
    ...
    report = {
        ...
        "teacher_label_distribution": {
            field: dict(sorted(counter.items())) for field, counter in labels.items()
        },
