"""Checkpoint archive / ranking utilities for the training pipeline."""
...
class CheckpointArchive:
    """In-memory index of the top checkpoints, persistable as JSON."""
    ...
    def add(self, path: Path, stage: str, env_steps: int, score: float) -> None:
        self.entries.append(
            CheckpointEntry(str(path), stage, int(env_steps), float(score), time.time())
        )
