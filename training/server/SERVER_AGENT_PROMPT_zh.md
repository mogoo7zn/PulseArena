...
3. 不要执行 `training/online_trainer.py`，不要用它生成或宣称 v2 模型。它输出原始 `PlayerAction`，会绕过 `HybridCombatExecutor`。
4. 不删除、不覆盖任何已有 replay、checkpoint、运行目录或归档。所有新产物写入 `training/artifacts/runs/`、`training/data/replays/` 或一个新的数据集目录。数据预处理脚本遇到非空输出目录会拒绝执行；此时换一个新目录，不要删除旧目录。
...
sed -n '1,260p' training/SERVER_AGENT_PROMPT_zh.md
export CUDA_VISIBLE_DEVICES=0
export GODOT_BIN=/absolute/path/to/godot
python training/preflight.py --godot "$GODOT_BIN" --require-cuda --output training/artifacts/runs/server_agent_preflight.json
```
...
只有全部返回 0 才进入下一阶段。将每个命令、退出码、关键 stdout/stderr 摘要写入 `training/artifacts/runs/server_agent_<UTC时间>/reports/command_log.md`。
...
```text
training/artifacts/runs/$RUN_ID/reports/
```

如果需要诊断，不改变源回放，重新执行：

```bash
python training/audit_hybrid_replays.py \
  --replay-dir training/replays \
  --output training/artifacts/runs/$RUN_ID/reports/raw_replay_audit.json \
  --min-rows 1000
python training/audit_hybrid_replays.py \
  --replay-dir training/replays_decision \
  --output training/artifacts/runs/$RUN_ID/reports/decision_replay_audit.json \
  --min-rows 1000
```
...
在 `training/artifacts/runs/$RUN_ID/reports/final_report.md` 中给出：
...
若且仅若 pilot 的数据、训练和报告完整，输出 `training/artifacts/runs/$RUN_ID/reports/tactical_ppo_implementation_plan.md`。计划必须要求新实现满足：
