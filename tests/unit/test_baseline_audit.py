...
class BaselineAuditTests(unittest.TestCase):
    ...
    def test_manifest_dimension_mismatch_fails(self):
        ...
        with TemporaryDirectory() as temporary:
            root = Path(temporary)
            for relative in ("training/models", "training/configs/training_plans", "training/core", "training/core/encoding", "scripts/agents"):
            ...
            (root / "training/core/encoding/observation.py").write_text("HYBRID_PROTOCOL_VERSION = 2\\nTACTICAL_FEATURE_SCHEMA_VERSION = 2\\n", encoding="utf-8")
            (root / "training/core/encoding/tactical.py").write_text("TACTICAL_FEATURE_DIM = 142\\n", encoding="utf-8")
