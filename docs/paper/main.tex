...
\begin{titlepage}
    ...
    \begin{minipage}{0.16\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/PulseArena-logo-pic.png}
    \end{minipage}
    \hfill
...
在获得高强度策略后，本文进一步设计了一套无需重训练的推理侧五档难度校准机制。该机制仅保留一个 PPO 训练得到的主 checkpoint，通过调节策略采样温度 $T$、动作掩码软化系数 $S_{\mathrm{soften}}$ 与安全兜底阈值 $\theta$，得到 Easy、Casual、Normal、Strong 和 Elite 五档行为强度。离线 self-play 对局结果显示，五档智能体相对于 \texttt{scripted\_normal}、\texttt{scripted\_hard} 与历史 \texttt{baseline v1} 三组固定对手均呈现稳定的胜率单调性。Normal 档的推理参数 $(T{=}0.85,\,S_{\mathrm{soften}}{=}0.10,\,\theta{=}0.75)$ 被设计为使真人对阵胜率落在预先设定的 $[0.50,0.75]$"有来有回"目标区间内；Elite 档则突破该区间上沿，体现为对高手玩家的显著挑战。然而，"强度是否合适"的最终判断需要真人对局参与的主观评分与接受率验证，该部分将在 Human Eval 阶段开展（详见 \S\ref{sec:human_eval}）。
...
\begin{enumerate}
    ...
    高层策略不再直接决定连续物理控制量，而是输出结构化战术意图；弹道、威胁规避、射击审计和卡死恢复等可解析或可规则化的问题交由低层确定性执行器完成。训练阶段采用行为克隆（Behavior Cloning, BC）进行温启动，并在 8 个独立 PPO worker 上并行扩训（各 worker 维护自己的 checkpoint；本项目未做 league 匹配或 DDP/MAPPO 同步 learner）。
\end{enumerate}
...
\begin{itemize}
    \item \textbf{面向 PulseArena 的分层混合控制架构。}
    ...
    \item \textbf{单 checkpoint 的推理侧五档难度校准机制。}
    使用温度、动作掩码软化与安全兜底阈值控制行为随机性、非最优动作概率和规则接管频率，从而避免为每个难度档分别维护独立模型。
    \item \textbf{建立客观对局与 Human Eval 接入位。}
    不仅在离线 self-play 中检查难度档位是否满足胜率单调性，还为真人对局的胜率、平均时长、主观评分与接受率预留 \texttt{training/evaluation/human\_eval\_harness.py} 接入通道，用于在后续阶段判断每档"强度是否合适"。当前阶段仅完成客观对局验证，Human Eval 数据采集尚未开展。
\end{itemize}
...
\subsection{地图结构}
默认标准地图为 \textit{Arena Cross}。地图中心设置十字型掩体，四角设置 L 型掩体。单局时长固定为 90 秒；时间结束时，以击杀数和剩余生命值等规则判定胜负。图~\ref{fig:arena_cross} 给出地图结构与两类典型射击关系：无遮挡视线下的预判射击，以及被中央掩体截断的视线。
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Arena Cross 地图结构及典型视线/预判射击关系。图中上半部分展示无遮挡情况下的移动目标截击，下半部分展示中央掩体造成的弹道阻断。}
...
在数轮诊断与扩训（约 $17\,\mathrm{k}$–$2.1\,\mathrm{M}$ 环境步）后，该方案仍存在三个稳定复现的问题。
...
在该阶段的诊断训练中，Protocol v1 端到端策略对 Scripted Hard 缺乏稳定优势。基于上述现象，我们停止继续扩大同一架构的训练规模，并将其作为 \texttt{archive/legacy\_raw\_ai/} 中的历史基线。
...
在 Protocol v2 稳定后，我们使用 8 个独立 PPO worker 在 8 张 A100 上并行扩训（各 worker 维护自己的 checkpoint；本项目未做 league 匹配或 DDP/MAPPO 同步 learner）。约 2.6M 环境步累计后得到的高强度策略在离线对局中表现优秀，但策略回放与参数敏感性分析指出两个可能影响真人对局可玩性的潜在问题。
...
\begin{itemize}
    ...
    \item \textbf{机器级规避精度。}
    TCA/DCA 与确定性执行器能够在非常短的时间尺度上做出一致反应。若安全覆盖层始终以最高灵敏度工作，回放中可观察到过度稳定的"最后时刻闪避"，从而产生明显的非人感。
\end{itemize}
...
\subsection{Human Eval 接入位（待执行）}\label{sec:human_eval}
本文阶段 5 的原始设计沿用 \texttt{docs/plan/05-strength-tier-and-human-eval.md} 的 Human Eval 方案：招募不少于 3 名内部测试员、各档不少于 50 局、4 张地图均覆盖、记录胜负、平均时长、5 分制主观评分，并将评分 $\ge3.5$ 的对局记为"接受"。Normal 档的预设备受区间为真人对阵胜率 $[0.50,0.75]$，作为本项目内部工程验收门槛，用于描述"玩家有较大胜算、但无法轻松获胜"的目标体验。该区间并非普适的人机体验标准，而是 PulseArena 当前内部测试阶段的工程约定。

\textbf{当前状态}：上述真人对局\textbf{尚未开展}。本文交付的 Human Eval 基础设施包括：

\begin{itemize}
    \item \texttt{training/evaluation/human\_eval\_harness.py}：提供 \texttt{record\_gameplay\_as\_replay}（人对局转 \texttt{hybrid\_replay\_v2} JSONL）、\texttt{replay\_filter}（质量门：\texttt{min\_game\_duration}=30s、丢弃仅触发 fallback 的局）、\texttt{strength\_tier\_monitor}（7 天滑窗监控并检测单调性 / 目标区间偏离）三个子命令；
    \item \texttt{scripts/core/human\_eval\_recorder.gd}：在 Godot 端采集并写入与 BC/PPO 训练兼容的 \texttt{hybrid\_replay\_v2} 行；
    \item \texttt{training/models/model\_catalog.json}：5 档 manifest（同一 checkpoint + 不同 \texttt{inference\_profile}）已登记，\texttt{serve\_agent} 启动后可按 \texttt{model\_id} 选择难度；
    \item 1 份 \texttt{record-smoke} 合成 JSONL（\texttt{training/data/replays/human\_eval\_continual/2026-08-17\_00-38-13\_dungeon\_s74103.hybrid\_v2.jsonl}），40 行固定合成数据，仅用于验证 harness 写入路径，不代表真实对局。
\end{itemize}

\textbf{未完成项}：

\begin{itemize}
    \item Godot 主菜单 5 档 UI 尚未接入（plan §3.3 要求的 \texttt{main\_menu.gd} \texttt{agent\_difficulty} 选项、\texttt{MatchConfig} \texttt{difficulty} 字段与 \texttt{remote\_agent\_controller.gd} 透传 \texttt{strength\_profile}）；
    \item 真实玩家数据未收集（计划要求 $\ge3$ tester、$\ge50$ 局/档、$\ge10$ 局/图）；
    \item 计划 §3.4 的"5 档之间胜率必须严格单调"门槛目前仅基于 \texttt{scripted\_normal / scripted\_hard / baseline v1} 离线 self-play 完成；Human Eval 数据用于补充"真人对阵胜率"维度的强度验证。
\end{itemize}

因此，本文中具体到某档的"真人胜率 $0.58\pm0.04$、平均主观评分 $4.45\pm0.08$、接受率 $91.8\%$"等数字\textbf{不作为已测结果呈现}；Normal 档的"打得有来有回"目前仅以 \texttt{[0.50,0.75]} 这一设计区间作为目标值说明，待 Human Eval 实际执行后填入真实数据。

\subsection{结果分析（仅基于客观对局）}
\paragraph{五档形成可区分的客观强度。}
表~\ref{tab:win_rate_results} 给出的胜率均来自 \texttt{evaluate\_tactical\_candidate.py --runner godot-service} 的离线 self-play 评估，不涉及真人参与。在 \texttt{scripted\_normal}、\texttt{scripted\_hard} 与 \texttt{baseline v1} 三组固定对手上，胜率均按 Easy $\rightarrow$ Elite 单调上升：Easy 全面处于劣势，Normal 形成稳定优势但未压倒，Strong 与 Elite 接近压倒性优势。该客观单调性说明推理侧三参数能够有效控制"竞技强度"这一维度。

\paragraph{主观体验维度未在本阶段验证。}
结合行为回放，Normal 档的 $T=0.85$ 与 $S_{\mathrm{soften}}=0.10$ 配置保留了稳定的战术执行能力，并引入了可感知的次优行为（停顿、低价值探头、射击资源使用偏差）。该设计是否在真人玩家中体现为"有来有回"，需要 Human Eval 阶段补齐主观评分与接受率后验证。Elite 档"过强"、Easy 档"过弱"对应的玩家偏好判断同理：本文在此不下结论，只给出参数设计与目标区间。
...
三参数机制的优点是工程成本低：无需维护五套 checkpoint，也无需为每档重新完成多 worker PPO 扩训（8 个 worker 各自 4k–2M 环境步不等）。然而它并不能替代真正的多能力策略学习。温度只能改变已有动作概率，无法创造网络从未学习过的战术；掩码软化只能引入更多次优尝试；安全阈值只能改变确定性系统的接管比例。因此，如果需要“新手 AI 会教学”“高手 AI 会诱骗”“某档 AI 具有不同角色风格”等语义层行为，仍需要额外条件化训练或多策略建模。

\subsection{当前实验的限制}
当前结果应被视为项目内验证，而不是对所有竞技游戏的普遍结论，主要限制包括：

\begin{itemize}
    \item \textbf{真人对局尚未开展，主观体验维度未验证。}
    本文交付的 Human Eval 仅为基础设施（harness + recorder + catalog + 1 份 smoke 数据），尚未收集任何真实玩家对局。本文不给出主观评分或接受率相关结论；§\ref{sec:human_eval} 列出了尚未完成的接入项。
    ...
    \item \textbf{当前评估主要集中于胜率单调性。}
    本文不包含"人类感"相关指标；后续应补充行为多样性、动作熵、掩体驻留比例、反应延迟分布、无效动作率等可解释行为指标。
\end{itemize}

这些限制也给出了下一阶段最直接的实验方向：先完成 Human Eval 接入与三参数消融，再扩大真人测试人群，并把"强度"和"行为风格"拆分为两个独立控制维度。
...
\subsection{工作总结}
本文围绕 PulseArena 的对战智能体设计，完成了从底层端到端控制到分层 Hybrid 控制的架构重构。Protocol v2 将目标选择、移动模式、射击模式与技能模式保留在学习策略中，把弹道截击、威胁几何分析、资源审计和卡死恢复交给确定性执行器。该设计减少了策略网络对低层物理规律的重复学习，并显著提升了控制稳定性。

在此基础上，本文进一步把"训练出强策略"与"部署合适难度"分开处理。通过温度 $T$、动作掩码软化 $S_{\mathrm{soften}}$ 和安全兜底阈值 $\theta$，同一主 checkpoint 被校准为五档强度。离线 self-play 对局显示三组固定基线上的胜率均随难度单调上升；Normal 档的推理参数 $(T{=}0.85,\,S_{\mathrm{soften}}{=}0.10,\,\theta{=}0.75)$ 被设计为使真人对阵胜率落在预先设定的 $[0.50,0.75]$ 目标区间内，但该区间在真实玩家群体中是否成立仍需 Human Eval 阶段验证。
