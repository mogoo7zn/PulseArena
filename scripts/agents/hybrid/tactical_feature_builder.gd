extends RefCounted
# Builds normalized v2 tactical features and masks from AgentObservation only.
...
const ProjectileThreatAnalyzer = preload("res://scripts/agents/hybrid/projectile_threat_analyzer.gd")
...
func build(obs: AgentObservation, balance: GameBalance, config: HybridAgentConfig, current_decision: HighLevelDecision = null) -> Dictionary:
	var features := PackedFloat32Array()
	var threat_info := threat_analyzer.analyze(obs, balance, config)
	var masks := build_action_masks(obs, balance, config, threat_info)
	_append_self_features(features, obs, balance, current_decision)
	_append_enemy_features(features, obs, balance)
...
func build_action_masks(obs: AgentObservation, balance: GameBalance, config: HybridAgentConfig, threat_info: Dictionary = {}) -> Dictionary:
	...
	for i in range(HighLevelDecision.FIRE_MODE_COUNT):
		fire_mask.append(any_enemy)
	fire_mask[HighLevelDecision.FireMode.HOLD_FIRE] = true
	fire_mask[HighLevelDecision.FireMode.USE_SCRIPTED_FIRE_MODE] = true
...
func _append_enemy_features(out: PackedFloat32Array, obs: AgentObservation, balance: GameBalance) -> void:
	if obs == null:
	...
	for i in range(AgentObservation.MAX_OTHER_PLAYERS):
		...
		var los := 1.0 if bool(p.get("valid", false)) and not bool(p.get("is_teammate", false)) else 0.0
