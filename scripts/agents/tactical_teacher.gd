...
const EPS := 0.00001
const ENEMY_FEATURE_START := 28
const ENEMY_FEATURE_WIDTH := 20
const ENEMY_TARGET_VISIBILITY_OFFSET := 14

func build_label(obs: AgentObservation, tactical_data: Dictionary, balance: GameBalance, diagnostics: Dictionary = {}, agent_config: HybridAgentConfig = null) -> Dictionary:
	...
	var target_info := _target_info(obs, tactical_data)
	var has_enemy := bool(target_info.get("has_enemy", false))
	var threat_level := clampf(float(threat_info.get("threat_level", 0.0)), 0.0, 1.0)
	var high_threat := threat_level >= (agent_config.high_threat_threshold if agent_config != null else 0.72)
	var emergency := threat_level >= (agent_config.emergency_threat_threshold if agent_config != null else 0.88)
	if not has_enemy:
		decision.target_slot = HighLevelDecision.TargetSlot.NONE
		decision.fire_mode = HighLevelDecision.FireMode.HOLD_FIRE
		...
		var target_visible := _selected_target_visible(decision, target_info)
		_assign_movement(decision, obs, target_info, target_visible, threat_level, high_threat, emergency, agent_config)
		_assign_fire(decision, obs, target_info, target_visible, threat_info, balance, high_threat, emergency)
...
func _assign_movement(decision: HighLevelDecision, obs: AgentObservation, target_info: Dictionary, target_visible: bool, threat_level: float, high_threat: bool, emergency: bool, agent_config: HybridAgentConfig) -> void:
	if high_threat:
		decision.movement_mode = HighLevelDecision.MovementMode.EVADE_PROJECTILE
		return
	# A map-blocked target is not an actionable shooting opportunity.  CHASE
	# activates the pressure profile's persistent lateral vantage search.
	if not target_visible:
		decision.movement_mode = HighLevelDecision.MovementMode.CHASE
		return
	var distance := float(target_info.get("closest_distance", 1.0))
	var cover_health_threshold := agent_config.cover_health_threshold if agent_config != null else 0.34
	var cover_threat_threshold := agent_config.cover_threat_threshold if agent_config != null else 0.0
	var cover_under_pressure := cover_threat_threshold <= 0.0 or threat_level >= cover_threat_threshold
	if obs.health_ratio < cover_health_threshold and cover_under_pressure:
		...
		# Pressure training should recover a usable fighting radius, not encode
		# an unconditional escape reflex after a successful close-in.
		decision.movement_mode = HighLevelDecision.MovementMode.KEEP_RANGE if _is_pressure_profile(target_info) else HighLevelDecision.MovementMode.RETREAT
	elif emergency:
		decision.movement_mode = HighLevelDecision.MovementMode.EVADE_PROJECTILE
	else:
		decision.movement_mode = HighLevelDecision.MovementMode.KEEP_RANGE

func _assign_fire(decision: HighLevelDecision, obs: AgentObservation, target_info: Dictionary, target_visible: bool, threat_info: Dictionary, balance: GameBalance, high_threat: bool, emergency: bool) -> void:
	var distance := float(target_info.get("closest_distance", 1.0))
	var lowest_health := float(target_info.get("lowest_health", 1.0))
...
func _target_info(obs: AgentObservation, tactical_data: Dictionary) -> Dictionary:
	var closest_slot := -1
	var closest_distance := INF
	var lowest_slot := -1
	var lowest_health := INF
	var visibility_by_slot: Dictionary = {}
	for i in range(mini(AgentObservation.MAX_OTHER_PLAYERS, obs.other_players.size())):
		...
		if not _is_valid_enemy(candidate):
			continue
		visibility_by_slot[i] = _feature_target_visibility(tactical_data, i)
	...
	return {
		...
		"visibility_by_slot": visibility_by_slot,
		"engagement_profile_id": str(tactical_data.get("engagement_profile_id", "")),
	}

func _selected_target_visible(decision: HighLevelDecision, target_info: Dictionary) -> bool:
	var slot := int(target_info.get("closest_slot", -1))
	if decision.target_slot == HighLevelDecision.TargetSlot.LOWEST_HEALTH_ENEMY:
		slot = int(target_info.get("lowest_slot", -1))
	if slot < 0:
		return false
	var visibility_by_slot: Dictionary = target_info.get("visibility_by_slot", {})
	return bool(visibility_by_slot.get(slot, true))

func _is_pressure_profile(target_info: Dictionary) -> bool:
	return str(target_info.get("engagement_profile_id", "")) == "legal_window_pressure"

func _feature_target_visibility(tactical_data: Dictionary, slot: int) -> bool:
	var features = tactical_data.get("features", PackedFloat32Array())
	var index := ENEMY_FEATURE_START + slot * ENEMY_FEATURE_WIDTH + ENEMY_TARGET_VISIBILITY_OFFSET
	if features == null or index < 0 or index >= features.size():
		# Older/default feature producers retain the legacy valid-enemy semantic.
		return true
	return float(features[index]) >= 0.5
